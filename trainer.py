...
class BaseTrainer(LightningModule):
    ...
    def load(self):
        ...
        #     print('skip loading embedding.weight')
        #     del state_dict['embedding.weight']
        
        u2m_keys = [k for k in state_dict if k.startswith('u2m.')]
        for k in u2m_keys:
            del state_dict[k]
        print(f"Skipped Unit2Mel ({len(u2m_keys)} params)")
        print('###############################################')
...
class RVectorTrainer(BaseTrainer):
    ...
    def run(self, dedup_unit, unit, rv, spkr, spkr_r, dur, voiced, batch):
        ...
        # print(batch['f0'].shape, f0_preds.shape, inp_f0.shape)
        # exit('batch[\'f0\'].shape, f0_preds.shape, inp_f0.shape') 
        # torch.Size([3, 706, 200]) torch.Size([3, 706, 200]) torch.Size([3, 706, 200])
        pred_mels_1, pred_mels_2, pred_mels_3, pred_mels_4 = self.u2m(inp_E, inp_f0, batch['voiced'].bool(), spkr, unit, batch['mel_mask'], batch['mel'].size(1), spkr_r=spkr_r)
        return pred_mels_1, pred_mels_2, pred_mels_3, pred_mels_4, duration_preds, f0_preds, voiced_preds, E
    ...
    def forward(self, batch):
        ...
        pred_mels_1, pred_mels_2, pred_mels_3, pred_mels_4, duration_preds, f0, voiced_preds, E = self.run(dedup_unit, unit, rv, speaker_embedding, speaker_embedding_real, dur, voiced, batch)
        pred_mels = pred_mels_1 + pred_mels_2 + pred_mels_3 + pred_mels_4
        #Duration prediction
        ...
        #MelSpec prediction
        mel_loss = F.l1_loss(pred_mels_1[~batch['mel_mask']], batch['mel_1'][~batch['mel_mask']])
        mel_loss += F.l1_loss(pred_mels_2[~batch['mel_mask']], batch['mel_2'][~batch['mel_mask']])
        mel_loss += F.l1_loss(pred_mels_3[~batch['mel_mask']], batch['mel_3'][~batch['mel_mask']])
        mel_loss += F.l1_loss(pred_mels_4[~batch['mel_mask']], batch['mel_4'][~batch['mel_mask']])
    ...
    def infer(self, batch, shuffled_audio=None, shuffled_mask=None, shuffle_target=None):
        ...
        #MelSpec Prediction
        pred_mels_1, pred_mels_2, pred_mels_3, pred_mels_4 = self.u2m(E, f0, voiced, speaker_embedding, unit, ~pred_mask)
        pred_mels = pred_mels_1 + pred_mels_2 + pred_mels_3 + pred_mels_4
